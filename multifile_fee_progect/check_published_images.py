...
def one_day_images_cycle(images_voc, i, path_to_file, count):
    print(f"{red} in  {i} - {len(images_voc)} were send to publication{end}")
    # count = 0
    for k in images_voc:
        count += 1
        used_images = publication_info(k, count, i)
        # write_to_file(path_to_file, used_images, count, i)
        write_xlsx_single_sheet(path_to_file, used_images)
    return count
